...
<%
	...
	// cos.jar 안에 존재하는 multipartRequest를 사용해야 한다.
	// String name = request.getParameter("name");
	// System.out.println("name : "+name);
	
	// cos.jar 안에 있는 nultipartRequest 클래스의 주요 메소드
	// - getParameter(String) : String
	// - getParameterValues(String) : String[]
			
	// (String) 인자는 input type="file" name="name1" 중 name1
	...
	//		- .getName() : 파일의 이름
	//		- .getLength() : 파일의 크기를 byte처리
	//		- .lastModified : 마지막 수정 날짜
	
	// 1. 실제 파일이 저장될 위치를 구하자(톰켓안에 프로젝트 안에 위치)
	...
	MultipartRequest mr = new MultipartRequest(
				...
				"utf-8", //인코딩
				new DefaultFileRenamePolicy()); // 파일이름 
...
<body>
	<h1>업로드된 정보 보기</h1>
	<h2>
		<li>올린 사람 : <%=mr.getParameter("mame") %></li>
		...
		<%-- 위 숫자는 1970.01.01.0.0.0 부터 현재날짜_시간 까지 날짜를 수치화 한것 --%>
		<%
		...
		<li>다운로드 : 
			<a href="download.jsp?path=upload&f_name=<%=file.getName()%>"></a>
		</li>
